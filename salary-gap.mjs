...
// Paths resolve under $CAREER_OPS_DATA_DIR (default ft/); config/profile.yml is shared root.
const P = resolvePaths(import.meta.url);
const CAREER_OPS = P.root;
const OBS_PATH = join(P.dataDir, 'salary-observations.tsv');
...
function collectSources() {
  ...
  if (existsSync(REPORTS_DIR)) {
    // Reports live one level deep under $CAREER_OPS_DATA_DIR: reports/{company}/
    // {NNN}-{role}-{date}.md. Walk each company folder (plus any flat legacy
    // files) so advertised_comp folding sees every report, not just top-level.
    for (const ent of readdirSync(REPORTS_DIR, { withFileTypes: true })) {
      const files = ent.isDirectory()
        ? readdirSync(join(REPORTS_DIR, ent.name)).map((f) => [join(REPORTS_DIR, ent.name, f), f])
        : [[join(REPORTS_DIR, ent.name), ent.name]];
