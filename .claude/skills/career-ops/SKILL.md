...
Available commands:
  ...
  npm run scan:all      → Discovery only across every active source (no eval; zero LLM tokens)
  npm run evaluate      → Manual evaluate of scan-results (provider-backed; see README Manual workflow)
...
invocation to consume. For a manual discovery-only pass without an agent eval
loop, run `npm run scan:all` (see modes/scan.md / docs/SCRIPTS.md). Any new
/career-ops invocation MUST first check for pre-existing
ft/data/scan-results-*.tsv files and resume evaluation against them before
doing anything else. Pasting URLs to /career-ops triggers auto-pipeline on
each immediately (no scan step).
```
...
Applies to: `auto-pipeline`, `offer`, `offers`, `pdf`, `contact`, `apply`, `batch`

### Standalone modes (only their mode file):
Read `modes/{mode}.md`

Applies to: `scan`, `tracker`, `deep`, `training`, `project`, `patterns`, `followup`, `interview-prep`, `gmail-sweep`, `outreach`

### Modes delegated to subagent:
For `apply` with Playwright, launch an Agent with the content of `_shared.md` +
`modes/apply.md` injected into the subagent prompt. `scan` is standalone and
uses `modes/scan.md` as its complete contract. Scan agents in default mode
must complete the inline evaluation pass before returning, with no leftover
candidates in `ft/data/scan-results-{date}.tsv`. Before dispatching any eval
agent, the scan agent MUST run the mandatory liveness gate over surviving URLs
and drop every URL classified `expired`; this gate cannot be skipped. In
`scan-only` mode, the agent stops after the title-level filter, reports row
counts, and leaves the TSV on disk for a follow-up invocation.
