...
export function renderTrackerRow({
  ...
  // Application eligibility requires an explicit APPLY or CONSIDER token in
  // the fixed nine-column tracker schema. The runner reads the tracker row
  // during its fast pre-open gate; a committed Consider report is also
  // enqueueable. REVIEW_REQUIRED and DO_NOT_APPLY never enter the queue.
  const actionNote = decision.decision === 'APPLY' ? 'APPLY.'
    : decision.decision === 'CONSIDER' ? 'CONSIDER.'
      : '';
