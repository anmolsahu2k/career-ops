...
export function renderEvaluationReport({ task, decision, presentation, reportNumber, trueAgeDays = null }) {
  ...
  const lines = [
    ...
    `**True age:** ${Number.isFinite(trueAgeDays) ? `${trueAgeDays}d` : 'unknown'}`,
    '',
  ];
...
/**
 * Notes tokens the mode contract expects on every row, so runtime-written rows
 * read the same as agent-written ones: liveness flag, true age, resume pick.
 */
function contextNotes({ subject, decision, trueAgeDays, date }) {
  const tokens = [];
  if (decision.gate_resolution?.posting_live?.value === 'UNKNOWN') {
    tokens.push(`LIVENESS-UNCERTAIN ${date}.`);
  }
  tokens.push(Number.isFinite(trueAgeDays) ? `TRUE-AGE: ${trueAgeDays}d.` : 'TRUE-AGE: unknown.');
  // Off-target rows get no resume pick, matching the `N/A (off-target)` rule.
  const offTarget = decision.decision === 'DO_NOT_APPLY'
    && decision.score !== null
    && decision.score <= 2;
  if (!offTarget && (subject.resume === 'SDE' || subject.resume === 'MLE')) {
    tokens.push(`Submit ${subject.resume} resume.`);
  }
  return tokens;
}

export function renderTrackerRow({
  task,
  decision,
  reportNumber,
  reportRelativePath,
  ...
  // Application eligibility deliberately requires an explicit APPLY token in
  // the fixed nine-column tracker schema. The rendered report already states
  // the recommendation, but the runner reads only the tracker row during its
  // fast pre-open gate. Preserve any review note and add the token only for a
  // final APPLY decision; CONSIDER and REVIEW_REQUIRED must never enter the
  // unattended queue by implication.
  const actionNote = decision.decision === 'APPLY' ? 'APPLY.' : '';
  const notes = withSrcToken([
    ...contextNotes({ subject, decision, trueAgeDays, date }),
    actionNote,
    reviewNote(decision),
