...
import { addUsage, divideUsage, normalizeUsage } from './usage.mjs';
...
function successResult(item, evaluation, {
  usage = {}, usageTrace = [], attempts = 1, latencyMs = 0, providerRun = 0,
} = {}) {
  ...
  return {
    ...
    contradictory_dimensions: evaluation.normalized.decision_inputs.contradictory_dimensions || [],
    ...
    total_tokens: token(usage, 'total_tokens') || token(usage, 'input_tokens') + token(usage, 'output_tokens'),
    measurement_quality: normalizeUsage(usage).measurement_quality,
    usage_trace: usageTrace,
    response_digest: sha256(evaluation.rawResult.response),
  };
}

function failureResult(item, error, {
  attempts = 2, providerRun = 0, usage = {}, usageTrace = [], latencyMs = 0,
} = {}) {
  const expectedGates = item.expected_gates || {};
  return {
    case_id: item.id,
...
const RECOMMENDATIONS = new Set(['APPLY', 'CONSIDER', 'DO_NOT_APPLY']);

export function reinterpretHistoricalRecommendationTruth({
  definition,
  run,
  attestationId,
  labelsPersonallyReviewed = false,
  now = new Date(),
}) {
  assertSet(definition);
  if (!attestationId) throw new Error('Explicit recommendation-scope attestation is required');
  if (labelsPersonallyReviewed !== true) {
    throw new Error('Recommendation-scope override requires labels personally reviewed by the user');
  }
  if (definition.schema !== 'RuntimePreparedQualificationSetV1'
      || definition.source_label_semantics !== 'FINAL_HISTORICAL_OUTCOME'
      || definition.label_scope?.length !== 1
      || definition.label_scope[0] !== 'historical_final_outcome') {
    throw new Error('Scope override requires final historical outcome labels');
  }
  const blockers = new Set(definition.promotion_blockers || []);
  if (!blockers.has('RECOMMENDATION_ONLY_LABELS') || !blockers.has('SPLIT_LABEL_REVIEW_REQUIRED')) {
    throw new Error('Scope override requires both historical label blockers');
  }
  const unresolved = [...blockers]
    .filter(blocker => !['RECOMMENDATION_ONLY_LABELS', 'SPLIT_LABEL_REVIEW_REQUIRED'].includes(blocker));
  if (unresolved.length) throw new Error(`Scope override cannot clear blockers: ${unresolved.join(', ')}`);
  if (run?.schema !== 'ShadowQualificationRunV1' || run.schema_version !== 1
      || run.qualification_set_digest !== definition.set_digest) {
    throw new Error('Source shadow is not bound to the prepared set');
  }
  if (run.truth_source !== definition.truth_source
      || run.label_scope?.length !== 1
      || run.label_scope[0] !== 'historical_final_outcome'
      || run.gate_labels_included !== false
      || run.promotion_eligible !== false) {
    throw new Error('Source shadow must be the unpromoted final-outcome diagnostic');
  }
  if (run.provider_id !== run.metrics?.provider_id || run.model_snapshot !== run.metrics?.model_snapshot) {
    throw new Error('Source shadow provider identity mismatch');
  }
  if (!Array.isArray(run.results) || run.results.length !== definition.case_count) {
    throw new Error('Source shadow must contain the complete prepared set');
  }
  const sourceCases = new Map(definition.cases.map(item => [item.id, item]));
  if (sourceCases.size !== definition.case_count) throw new Error('Prepared set contains duplicate case IDs');
  const seen = new Set();
  const results = run.results.map(result => {
    const source = sourceCases.get(result.case_id);
    if (!source || seen.has(result.case_id)) throw new Error('Source shadow contains missing or duplicate case IDs');
    seen.add(result.case_id);
    if (source.comparison_stage !== 'DIAGNOSTIC_FINAL_OUTCOME'
        || source.label_semantics !== 'FINAL_HISTORICAL_OUTCOME'
        || source.expected_recommendation !== result.expected_recommendation) {
      throw new Error(`Source shadow label mismatch for ${result.case_id}`);
    }
    if (!RECOMMENDATIONS.has(result.advisory_recommendation)) {
      throw new Error(`Source shadow lacks an advisory recommendation for ${result.case_id}`);
    }
    return {
      ...result,
      recommendation_comparison_eligible: true,
      actual_recommendation: result.advisory_recommendation,
    };
  });
  const evaluationSetVersion = `${definition.evaluation_set_version}-recommendation-scope-v1`;
  const metadata = {
    provider_id: run.provider_id,
    model_snapshot: run.model_snapshot,
    task_class: run.metrics.task_class,
    capability_class: run.metrics.capability_class,
    evaluation_set_version: evaluationSetVersion,
    representative_set: true,
    truth_source: 'HUMAN_APPROVED_HISTORY',
    label_scope: ['advisory_recommendation'],
    gate_labels_included: false,
    promotion_eligible: false,
  };
  const metrics = aggregateQualificationResults(results, metadata);
  const componentPassed = results.length >= 50
  ...
  return record('ShadowQualificationRunV1', {
    ...
    gate_labels_included: false,
    promotion_eligible: false,
    promotion_blockers: ['RECOMMENDATION_ONLY_LABELS'],
    component_passed: componentPassed,
    scope_override: record('HistoricalRecommendationScopeOverrideV1', {
      source_prepared_set_digest: definition.set_digest,
      source_shadow_digest: sha256(run),
      authority: 'user',
      label_provenance: 'USER_PERSONALLY_REVIEWED',
      method: 'explicit_personal_review_attestation_without_second_per_case_review',
      attestation_digest: sha256(attestationId),
      accepted_at: now.toISOString(),
    }),
    offset: run.offset,
    ...(run.selected_case_ids ? { selected_case_ids: [...run.selected_case_ids] } : {}),
    requested_count: run.requested_count,
    planned_provider_run_count: run.planned_provider_run_count,
    fallback_provider_run_count: run.fallback_provider_run_count,
    provider_run_count: run.provider_run_count,
...
function includePriorConsumption(result, prior) {
  const usage = addUsage(result, prior.usage);
  return {
    ...result,
    ...
    usage_trace: [...(prior.usageTrace || []), ...(result.usage_trace || [])],
    ...(prior.error ? {
...
async function evaluateBatch(batch, provider, providerRun, { repairFrom = null } = {}) {
  ...
  for (let attempt = 1; attempt <= maximumAttempts; attempt++) {
    try {
      ...
      const trace = {
        attempt,
        request: repairFrom || attempt > 1 ? 'REPAIR' : 'INITIAL',
        usage: attemptUsage,
        latency_ms: Number(raw.latency_ms || 0),
        schema_result: 'PENDING',
        allocation: batch.length > 1 ? 'BATCH_EQUAL_SHARE' : 'EXACT_REQUEST',
      };
      ...
      const allocatedTrace = usageTrace.map(entry => ({
        ...entry,
      ...
      for (const item of batch) {
        try {
          ...
          results.push(successResult(item, evaluation, {
            ...
            providerRun,
            usageTrace: allocatedTrace,
          }));
        } catch (error) {
          failures.push({
            item,
            error,
            prior: { usage: allocatedUsage, usageTrace: allocatedTrace, attempts: attempt, latencyMs: accumulatedLatency, error },
          });
        }
      }
      ...
      trace.schema_result = failures.length ? 'PARTIAL' : 'VALID';
      allocatedTrace.at(-1).schema_result = trace.schema_result;
      return { results, failures, attempts: attempt, requestTrace: usageTrace };
    } catch (error) {
  ...
  throw Object.assign(lastError, {
    ...
    requestTrace: usageTrace,
    prior: {
      usage: divideUsage(accumulatedUsage, batch.length),
      usageTrace: usageTrace.map(entry => ({ ...entry, usage: divideUsage(entry.usage, batch.length) })),
      attempts: maximumAttempts,
      latencyMs: accumulatedLatency,
    },
  });
...
export async function runShadowQualification({
  ...
  if (taskTransform) cases = cases.map(item => ({ ...item, task: taskTransform(item.task) }));
  ...
  for (let index = 0; index < batches.length; index++) {
    ...
    if (batch.length === 1) {
      ...
      try {
        ...
        providerRequests.push(...(evaluation.rawResult.usage_trace || []).map(trace => ({
        ...
        results.push(successResult(item, evaluation, {
          ...
          providerRun: index + 1,
          usageTrace: evaluation.rawResult.usage_trace || [],
        }));
      } catch (error) {
        providerCallCount += error.attempts || 2;
        providerRequests.push(...(error.usageTrace || []).map(trace => ({ provider_run: index + 1, ...trace })));
        results.push(failureResult(item, error, {
          attempts: error.attempts || 2,
          providerRun: index + 1,
          usage: error.usage || {},
          usageTrace: error.usageTrace || [],
      ...
      try {
        ...
        providerRequests.push(...outcome.requestTrace.map(trace => ({ provider_run: index + 1, ...trace })));
        ...
        for (const failed of outcome.failures) {
          ...
          try {
            ...
            providerRequests.push(...fallback.requestTrace.map(trace => ({ provider_run: fallbackRun, ...trace })));
            if (fallback.results.length === 1) {
              ...
              results.push(failureResult(failed.item, fallbackFailure.error, {
                ...
                usageTrace: [...(failed.prior.usageTrace || []), ...(fallbackFailure.prior.usageTrace || [])],
              }));
            }
          } catch (fallbackError) {
            providerCallCount += fallbackError.attempts || 2;
            providerRequests.push(...(fallbackError.requestTrace || []).map(trace => ({ provider_run: fallbackRun, ...trace })));
            ...
            results.push(failureResult(failed.item, fallbackError, {
              ...
              usageTrace: [...(failed.prior.usageTrace || []), ...(prior.usageTrace || [])],
            }));
          }
        }
      } catch (error) {
        providerCallCount += error.attempts || 2;
        providerRequests.push(...(error.requestTrace || []).map(trace => ({ provider_run: index + 1, ...trace })));
          ...
          try {
            ...
            if (fallback.results.length === 1) {
              ...
              results.push(failureResult(item, fallbackFailure.error, {
                ...
                usageTrace: [...(error.prior.usageTrace || []), ...(fallbackFailure.prior.usageTrace || [])],
              }));
            }
          } catch (fallbackError) {
            providerCallCount += fallbackError.attempts || 2;
            ...
            results.push(failureResult(item, fallbackError, {
              ...
              usageTrace: [...(batchPrior.usageTrace || []), ...(fallbackPrior.usageTrace || [])],
            }));
          }
        }
      }
