...
export function assertPolicyDecision(value) {
  ...
  if (isPlainObject(value)) {
    ...
    if (requiredObject(value.gate_resolution, '$.gate_resolution', issues)) {
      for (const gate of GATES) {
        const item = value.gate_resolution[gate];
        if (requiredObject(item, `$.gate_resolution.${gate}`, issues)) {
          enumValue(item.value, TRI_STATES, `$.gate_resolution.${gate}.value`, issues);
          if (!Array.isArray(item.evidence_refs)) {
            issues.push(`$.gate_resolution.${gate}.evidence_refs must be an array`);
          }
        }
      }
      for (const key of Object.keys(value.gate_resolution)) {
        if (!GATES.includes(key)) issues.push(`$.gate_resolution.${key} is not an allowed gate`);
      }
    }
    if (!Array.isArray(value.authorized_writes)) {
      issues.push('$.authorized_writes must be an array');
    } else {
      for (const write of value.authorized_writes) {
        if (!AUTHORIZED_WRITES.includes(write)) {
          issues.push(`$.authorized_writes contains unauthorized write '${write}'`);
        }
      }
      if (value.decision === 'DEFERRED' && value.authorized_writes.length) {
        issues.push('$.authorized_writes must be empty when decision is DEFERRED');
      }
      if (value.decision !== 'DEFERRED' && value.authorized_writes.length === 0) {
        issues.push('$.authorized_writes must authorize persistence when decision is not DEFERRED');
      }
    }
