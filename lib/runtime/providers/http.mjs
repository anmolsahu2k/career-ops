...
// `free_tier` is a per-provider declaration that the key incurs no charges
// (Cerebras / Groq free keys). It is an explicit opt-in like `local_only`, so a
// zero-cost endpoint does not require flipping the global api_billing switch.
function apiAllowed(config) {
  return config.enabled === true && (
    config.local_only === true
    || config.free_tier === true
    || config.api_billing_enabled === true
  );
}
...
export class HttpProvider {
  ...
  async complete(request, { attempt = 1, repair = null } = {}) {
    ...
    if (this.config.api_style === 'anthropic_messages') {
      ...
      body = {
        model: this.config.model_snapshot,
        messages: [{ role: 'user', content: prompt }],
        response_format: { type: 'json_object' },
        ...(this.config.temperature !== undefined ? { temperature: Number(this.config.temperature) } : {}),
        ...(this.config.max_output_tokens !== undefined ? { max_tokens: Number(this.config.max_output_tokens) } : {}),
      };
    }
    ...
    return record('RawProviderResultV1', {
      ...
      response: output || '',
      usage: normalizeUsage(payload.usageMetadata || payload.usage || {}),
