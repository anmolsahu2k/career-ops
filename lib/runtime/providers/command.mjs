...
export function resolveSchemaPath(value, targetPlatform = process.platform) {
  const raw = String(value);
  // URL.pathname uses /C:/... for Windows file URLs. path.resolve treats that
  // as a rooted path on the current drive and can produce C:\\C:\\..., so
  // normalize the URL-style drive prefix before resolving.
  const native = targetPlatform === 'win32' && /^\/[A-Za-z]:[\\/]/.test(raw)
    ? raw.slice(1)
    : raw;
  return targetPlatform === 'win32' ? win32.resolve(native) : resolve(native);
}

export function allowedEnvironment(names = [], environment = process.env, targetPlatform = process.platform) {
  const normalize = targetPlatform === 'win32' ? value => value.toUpperCase() : value => value;
  ...
  return Object.fromEntries(Object.entries(environment).filter(([name]) => (
    allowed.has(normalize(name)) || name.startsWith('CAREER_OPS_')
  )));
}
...
export function parseDurationMs(value) {
  ...
  if (!match) return 0;
  return Number(match[1]) * (DURATION_UNITS[match[2] || 's'] || 0);
}

export function printTimeoutMs(command = []) {
  const args = Array.isArray(command) ? command : [];
  for (let index = 0; index < args.length - 1; index++) {
    if (args[index] === '--print-timeout') return parseDurationMs(args[index + 1]);
  }
  return 0;
}

export function resolveCommandTimeoutMs(config = {}, { defaultMs = 120_000, bufferMs = 30_000 } = {}) {
  const configured = Number(config.timeout_ms);
  const fromConfig = Number.isFinite(configured) && configured > 0 ? configured : defaultMs;
  const fromFlag = printTimeoutMs(config.command);
  return Math.max(fromConfig, fromFlag > 0 ? fromFlag + bufferMs : 0);
}

function decodeOutput(stdout, mode, stderr = '') {
  ...
  if ((response === '' || response == null) && /no output produced|auto-denied|cannot prompt/i.test(stderr)) {
    throw new Error(String(stderr).trim().split(/\r?\n/)[0].slice(0, 240));
  }
...
function safeStderrDetail(stderr) {
  const diagnostics = String(stderr || '').split(/\r?\n/)
    ...
    .filter(line => /^(?:error|fatal|warning)(?:\b|:)/i.test(line)
      || /auto-denied|cannot prompt|no output produced/i.test(line))
    .slice(-8)
...
function stderrUsage(stderr, mode) {
  ...
  return Number.isFinite(total) && total > 0
    ? { total_tokens: total, measurement_quality: 'REPORTED_TOTAL_ONLY' }
    : {};
}
...
export class CommandProvider {
  constructor(config) {
    ...
    if (config.json_schema_file) {
      const schemaPath = resolveSchemaPath(config.json_schema_file);
      const schemaArgument = config.json_schema_mode === 'path'
        ? schemaPath
        : readFileSync(schemaPath, 'utf8').replace(/^\uFEFF/, '').replace(/\r\n/g, '\n');
      const schemaArgs = [config.json_schema_flag || '--json-schema', schemaArgument];
      const printIdx = Math.max(this.command.lastIndexOf('--print'), this.command.lastIndexOf('-p'));
      const stdinPromptIndex = this.command.at(-1) === '-' ? this.command.length - 1 : -1;
      if (printIdx >= 0) this.command.splice(printIdx, 0, ...schemaArgs);
      else if (stdinPromptIndex >= 0) this.command.splice(stdinPromptIndex, 0, ...schemaArgs);
  ...
  close() {
    ...
    try {
      ...
      // Windows can keep an Antigravity/Codex isolated cwd locked after the
      // child exits. The temp directory is disposable either way.
    }
  ...
  async complete(request, { attempt = 1, repair = null } = {}) {
    ...
    return new Promise((resolvePromise, rejectPromise) => {
      ...
      const timeoutMs = resolveCommandTimeoutMs(this.config);
      ...
      child.on('error', error => {
        ...
        if (timedOut) {
          rejectPromise(Object.assign(
            new Error(`${basename(executable)} exceeded the ${timeoutMs}ms command deadline`),
      ...
      child.on('close', code => {
        ...
        try {
          const decoded = decodeOutput(stdout, this.config.output_mode, stderr);
          ...
          resolvePromise(record('RawProviderResultV1', {
            ...
            usage: normalizeUsage(Object.keys(decoded.usage || {}).length ? decoded.usage : observedUsage),
