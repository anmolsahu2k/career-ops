...
function validateConfig(config) {
  ...
  if (config.applications !== undefined) {
    ...
    if (applications.main_profile !== undefined) {
      ...
      if (mainProfile.user_data_dir !== undefined) {
        if (typeof mainProfile.user_data_dir !== 'string') {
          throw new Error('applications.main_profile.user_data_dir must be a string');
        }
        const dir = mainProfile.user_data_dir.trim();
        if (dir && !isAbsolute(dir)) {
          throw new Error('applications.main_profile.user_data_dir must be an absolute local path');
        }
      }
      if (mainProfile.browser_channel !== undefined) {
        if (!['chrome', 'chromium', 'msedge', 'edge'].includes(mainProfile.browser_channel)) {
          throw new Error('applications.main_profile.browser_channel must be chrome, chromium, or msedge');
        }
      }
      if (mainProfile.apply_score_minimum !== undefined) {
        const floor = Number(mainProfile.apply_score_minimum);
        if (!Number.isFinite(floor) || floor < 0 || floor > 5) {
          throw new Error('applications.main_profile.apply_score_minimum must be 0-5');
        }
      }
      if (mainProfile.handshake !== undefined) {
        const handshake = mainProfile.handshake;
        if (!handshake || typeof handshake !== 'object') {
          throw new Error('applications.main_profile.handshake must be an object');
        }
        if (handshake.search_url !== undefined) {
          let search;
          try { search = new URL(String(handshake.search_url)); } catch {
            throw new Error('applications.main_profile.handshake.search_url must be an https joinhandshake.com URL');
          }
          if (search.protocol !== 'https:' || !/(^|\.)joinhandshake\.com$/i.test(search.hostname)) {
            throw new Error('applications.main_profile.handshake.search_url must be an https joinhandshake.com URL');
          }
        }
      }
