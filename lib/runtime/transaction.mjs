...
export function buildCommitPlan({ paths, task, decision, presentation, transactionId, reportNumber, rawResult, normalized, writerIdentity = null, trueAgeDays = null }) {
  ...
  const report = renderEvaluationReport({ task, decision, presentation, reportNumber, trueAgeDays });
  const trackerRow = renderTrackerRow({ task, decision, reportNumber, reportRelativePath, date, trueAgeDays });
...
export async function commitEvaluation({ target, task, decision, presentation, rawResult = null, normalized = null, hooks = {}, trueAgeDays = null }) {
  const paths = persistencePaths(target);
  ensureRuntimeDirectories(paths);
  assertTaskEnvelope(task);
  ...
  try {
    ...
    const built = buildCommitPlan({
      ...
      rawResult, normalized, writerIdentity: lock.owner, trueAgeDays,
    });
