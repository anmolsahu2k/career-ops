...
import { usageTotal } from './usage.mjs';
...
export function aggregateQualificationResults(results, metadata = {}) {
  ...
  const totalTokens = results.reduce((sum, item) => sum + usageTotal(item), 0);
...
export function composeQualificationEvidence({ recommendationRun, hardGateRun, incumbent = null }) {
  ...
  if (recommendationRun.scope_override
      && recommendationRun.scope_override.label_provenance !== 'USER_PERSONALLY_REVIEWED') {
    throw new Error('recommendationRun scope override lacks personal-review label provenance');
  }
