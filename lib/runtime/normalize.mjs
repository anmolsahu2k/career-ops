...
function validateCandidateShape(candidate) {
  ...
  if (candidate.decision_inputs.contradictory_dimensions !== undefined
      && !Array.isArray(candidate.decision_inputs.contradictory_dimensions)) {
    throw new RuntimeError('SEMANTIC_INVALID', 'contradictory_dimensions must be an array');
  }
}
...
export function normalizeEvaluation(task, rawResult, { now = new Date().toISOString(), limits = DEFAULT_SANITIZER_LIMITS } = {}) {
  ...
  const contradictionValues = new Set(['MERITS', 'SENIORITY', 'COMPENSATION', 'ROLE_FIT']);
  const contradictoryDimensions = [...new Set(candidate.decision_inputs.contradictory_dimensions || [])]
    .filter(value => contradictionValues.has(value));
  ...
  const normalized = record('NormalizedEvaluationV1', {
    ...
    decision_inputs: {
      ...
      contradictory_dimensions: contradictoryDimensions,
    },
