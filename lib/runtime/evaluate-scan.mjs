...
export async function fetchJobEvidence(row, { fetchImpl = fetch, pageText = '', skipHttpFetch = false } = {}) {
  const handshakeHost = (() => {
    try { return /(^|\.)joinhandshake\.com$/i.test(new URL(row.url).hostname); }
    catch { return false; }
  })();
  const text = plainTextFromHtml(pageText || '');
  if ((handshakeHost || skipHttpFetch) && text.length >= MIN_JD_CHARS) {
    return {
      ok: true,
  ...
  if (handshakeHost || skipHttpFetch) {
    return {
      ok: false,
...
export async function evaluateScanResults({
  ...
  if (Array.isArray(rows)) {
    ...
    candidates = loadScanResults(selectedFiles);
    sourceMode = 'triage';
  }
  if (Array.isArray(urls) && urls.length) {
    const wanted = new Set(urls.map(item => String(item || '').trim()).filter(Boolean));
    candidates = candidates.filter(row => wanted.has(row.url));
  }
  if (Number.isFinite(max)) candidates = candidates.slice(0, max);
  if (fromQueue && !candidates.length && !(Array.isArray(urls) && urls.length)) {
    ...
    await runPool(toEvaluate, evaluateConcurrency, async ({ row, verdict }, index) => {
      ...
      try {
        ...
        onProgress?.({
          ...
          code: slots[index]?.code || null,
          error: slots[index]?.error || null,
        });
      }
    });
  } finally {
...
/**
 * Score one already-open posting through the same prepare → provider →
 * PolicyEngine → commit path as evaluate --apply. Callers supply page text
 * (Handshake CDP) so this never launches headless Chromium or anonymous HTTP
 * against an auth-walled host.
 */
export async function evaluateLivePosting({
  target,
  config = null,
  posting = {},
  pageText = '',
  ...
  const row = {
    ...
    source: posting.source || 'manual',
    file: '(live)',
  };
  return evaluateScanResults({
    target,
    config,
    apply: true,
    skipLiveness: true,
    ...
    fetchImpl,
    livenessCache: false,
    livenessResults: new Map([[row.url, {
      result: 'active',
      reason: 'live_page',
      bodyText: text,
      status: 'live',
    }]]),
    fetchEvidence: (item, opts) => fetchJobEvidence(item, {
      ...opts,
      fetchImpl,
      pageText: text || opts?.pageText || '',
      skipHttpFetch: true,
    }),
  });
}
...
function writeFailureLedger(paths, failedRows) {
  ...
  try {
    writeScanResultsTsv(path, failedRows.map(item => ({
      ...
      title: item.title,
      location: '',
      raw_source: [item.code || item.status, item.error]
        .filter(Boolean)
        .join(' | ')
        .replace(/\t/g, ' ')
        .slice(0, 240),
