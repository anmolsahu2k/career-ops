...
} from '../applications/apply-url.mjs';
...
function sourceFor(url) {
  ...
  if (/(^|\.)greenhouse\.io$/i.test(host) || host === 'grnh.se') return 'greenhouse';
...
function greenhouseContentUrl(board, jobId) {
  return `https://boards-api.greenhouse.io/v1/boards/${encodeURIComponent(board)}/jobs/${encodeURIComponent(jobId)}?content=true`;
}

async function fetchGreenhouseJob(sourceUrl, { company = '', fetchImpl } = {}) {
  const certified = certifiedGreenhouseApiEndpoint(sourceUrl);
  if (certified) {
    return { endpoint: certified, response: await fetchJson(fetchImpl, certified) };
  }
  const jobId = greenhouseJobId(sourceUrl);
  if (!jobId) return { endpoint: null, response: null };
  const guesses = guessGreenhouseBoardTokens(sourceUrl, { company });
  let last = { endpoint: null, response: null };
  for (const board of guesses) {
    const endpoint = greenhouseContentUrl(board, jobId);
    const response = await fetchJson(fetchImpl, endpoint);
    last = { endpoint, response };
    if (response.status === 200 && response.json?.title) return last;
  }
  return last;
}
 ...
 */
export async function probeAtsLiveness(sourceUrl, { fetchImpl = fetch, company = '' } = {}) {
  let sourceType;
  try {
  ...
  try {
    if (sourceType === 'greenhouse') {
      const { endpoint, response } = await fetchGreenhouseJob(sourceUrl, { company, fetchImpl });
      if (!endpoint || !response) return { result: 'unknown', reason: 'greenhouse_url_unparseable' };
...
export async function captureHistoricalEvidence({
  caseId,
  sourceUrl,
  expectedTitle,
  company = '',
  ...
  try {
    ...
    let postedAt = null;
    if (sourceType === 'greenhouse') {
      const fetched = await fetchGreenhouseJob(sourceUrl, { company, fetchImpl });
      endpoint = fetched.endpoint;
      if (!endpoint || !fetched.response) return unavailable(caseId, sourceUrl, sourceType, 'SOURCE_URL_UNPARSEABLE', now);
