...
import { mainProfileAtsEnabled } from '../applications/ats.mjs';
import { handshakeStatusSnapshot } from '../handshake/status.mjs';
...
const CSRF_HEX = /^[a-f0-9]{48}$/;

/** Same localhost token across UI restarts on this data root and port. */
export function loadOrCreateCsrfToken(target, port = DEFAULT_PORT) {
  const runtimeDir = persistencePaths(target).runtimeDir;
  mkdirSync(runtimeDir, { recursive: true });
  const portKey = Number.isFinite(Number(port)) ? String(Number(port)) : '0';
  const file = join(runtimeDir, `web-csrf-${portKey}`);
  try {
    const existing = String(readFileSync(file, 'utf8') || '').trim();
    if (CSRF_HEX.test(existing)) return existing;
  } catch { /* first listen on this port */ }
...
export function createCareerOpsApp({
  ...
  const csrfToken = loadOrCreateCsrfToken(absoluteTarget, port);
  ...
  const applyBoard = createApplyBoardHandler(absoluteTarget, {
    ...
    onRetry: mode.writable && mode.applicationsEnabled && mode.config
      ? async key => {
        ...
        await jobs.start('apply_retry', { key });
      }
      : null,
  });
  ...
  async function statusPayload() {
    ...
    try {
      status.handshake = await handshakeStatusSnapshot(live.config, { timeoutMs: 400, cached: true });
    } catch { /* keep config-only handshake snapshot */ }
    return status;
  }
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/api/apply/preview') {
        return json(res, 200, applicationQueuePreview(absoluteTarget, { config: writerMode(configPath, config, observedHost).config }));
      }
      ...
      if (req.method === 'GET' && url.pathname === '/api/events') {
        ...
        res.write(`data: ${JSON.stringify({ event: 'hello', job: jobs.getCurrent(), csrf: csrfToken })}\n\n`);
        sseClients.add(res);
      ...
      if (req.method === 'POST' && url.pathname === '/api/jobs') {
        ...
        if ((action === 'apply_run' || action === 'apply_row' || action === 'apply_retry'
          || action === 'handshake_job' || action === 'handshake_session') && !live.applicationsEnabled) {
          return json(res, 403, { error: 'applications.enabled must be true', code: 'APPLICATIONS_DISABLED' });
        }
        if ((action === 'handshake_job' || action === 'handshake_session')
          && !mainProfileAtsEnabled(live.config, 'handshake')) {
          return json(res, 403, {
            error: 'Handshake requires applications.main_profile.enabled and ats: [handshake]',
            code: 'HANDSHAKE_DISABLED',
          });
        }
