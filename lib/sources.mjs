...
/** Handshake Apply Externally destination, stored in Notes without a new column. */
export function readApplyUrlToken(notes) {
  const m = String(notes || '').match(/\bAPPLY-URL:\s*(https?:\/\/\S+)/i);
  return m ? m[1].replace(/[.,;:!?]+$/, '') : '';
}

export function readApplyHostToken(notes) {
  const href = readApplyUrlToken(notes);
  if (href) {
    try { return new URL(href).hostname.toLowerCase(); } catch { /* fall through */ }
  }
  const m = String(notes || '').match(/\bHOST:\s*([a-z0-9][a-z0-9.-]+\.[a-z]{2,})/i);
  return m ? m[1].toLowerCase() : '';
}

export function withApplyUrlToken(notes, url, host = '') {
  let body = String(notes || '').trim()
    .replace(/\s*\bAPPLY-URL:\s*\S+/gi, '')
    .replace(/\s*\bHOST:\s*[a-z0-9][a-z0-9.-]+\.[a-z]{2,}\.?/gi, '')
    .trim();
  ...
  const bits = [];
  if (hostname && !/(^|\.)joinhandshake\.com$/i.test(hostname)) bits.push(`HOST: ${hostname}`);
  if (href && !/(^|\.)joinhandshake\.com$/i.test(href)) bits.push(`APPLY-URL: ${href}`);
  if (!bits.length) return body;
  const token = `${bits.join('. ')}.`;
  if (!body) return token;
  return `${body}${/[.!?]$/.test(body) ? '' : '.'} ${token}`;
}
