...
import { routeTask } from '../runtime/router.mjs';
import { isoNow, newId, record, sha256 } from '../runtime/util.mjs';
...
function routingEnvelope(task) {
  const now = isoNow();
  return record('TaskEnvelopeV1', {
    task_id: task.task_id, created_at: now, task_class: 'application_answer', risk: 'HIGH',
    minimum_capability_class: 'CONSEQUENTIAL', required_capabilities: ['structured_output', 'evidence_citations'],
    rules: { deterministic_submission_authority: true },
    subject: { company: 'application', role: 'application answer', url: 'https://career-ops.local/application', resume: 'SDE', source: 'local' },
    evidence_manifest: task.evidence_manifest.map(item => record('EvidenceReferenceV1', {
      id: item.id, source_type: item.kind, uri: `file:///career-ops/${item.id}`, content_hash: sha256(item.text),
      retrieved_at: now, liveness_state: 'YES', trust_class: 'trusted_evidence', structured_fields: {}, spans: [],
    })),
    expected_output: { schema: 'ApplicationAnswerResponseV1' }, context_hashes: {}, idempotency_key: task.task_id,
  });
}

function salaryRoutingEnvelope(task) {
  const now = isoNow();
  return record('TaskEnvelopeV1', {
    task_id: task.task_id, created_at: now, task_class: 'application_salary_preference', risk: 'HIGH',
    minimum_capability_class: 'CONSEQUENTIAL', required_capabilities: ['structured_output', 'evidence_citations'],
    rules: { deterministic_submission_authority: true, prospective_salary_preference_only: true },
    subject: { company: 'application', role: 'salary preference', url: 'https://career-ops.local/application', resume: 'SDE', source: 'local' },
    evidence_manifest: task.evidence_manifest.map(item => record('EvidenceReferenceV1', {
      id: item.id, source_type: item.kind, uri: `file:///career-ops/${item.id}`, content_hash: sha256(item.text),
      retrieved_at: now, liveness_state: 'YES', trust_class: 'trusted_evidence', structured_fields: {}, spans: [],
    })),
    expected_output: { schema: 'ApplicationSalaryResponseV1' }, context_hashes: {}, idempotency_key: task.task_id,
  });
}
  ...
  try {
    ...
    const request = {
      ...
      instruction: [
        ...
        `Use this exact JSON shape, with one object for each requested field: ${JSON.stringify({ answers: localTask.questions.map(question => ({ field_id: question.field_id, text: 'concise grounded answer', evidence_ids: [localTask.evidence_manifest[0]?.id || 'missing-evidence'], confidence: 0.8, claims_validated: false })) })}`,
        ...
        ...(hasCoverLetter ? [
          ...
          'Map concrete candidate proof from trusted-local evidence to specific needs in the current-job-report. Cite at least one evidence ID of each kind.',
    ...
    try {
      ...
      // One exact-format repair is permitted. A second malformed response is a
      // review blocker rather than an unbounded local generation loop.
      const repair = await provider.complete(request, {
        attempt: 2,
        repair: { error: String(error.message).slice(0, 240), instruction: hasCoverLetter
          ? 'Return one answer for every requested field with the exact field_id and only listed evidence_ids. For COVER_LETTER, write exactly 4 paragraphs of 30-40 words each, under 180 words total. Use blank lines between paragraphs. Body only: no header, greeting, closing, sign-off, links, visa, sponsorship, or availability. Use plain ASCII punctuation and only supplied facts.'
...
export async function generateBoundedAnswers({ questions, evidence, runtimeConfig, voiceProfile = '', requireLocal = false }) {
  ...
  const local = await generateLocalAnswers(task, runtimeConfig);
  ...
  const allowedProviders = hostedFallbackProviderIds(runtimeConfig);
  if (!allowedProviders.length) return { route: { result: 'NO_ELIGIBLE_PROVIDER', reason: 'PROVIDER_UNAVAILABLE' }, answers: [], blocker: 'PROVIDER_UNAVAILABLE' };
  const route = routeTask(routingEnvelope(task), runtimeConfig, { providerIds: allowedProviders });
  if (route.result !== 'ROUTED') return { route, answers: [], blocker: route.reason };
  const provider = createProvider(route.provider_id, {
    ...runtimeConfig.providers[route.provider_id],
    // Evaluation schemas cannot validate candidate-facing answer envelopes.
    json_schema_file: new URL('../../schemas/runtime/application-answer-response.v1.schema.json', import.meta.url).pathname,
  }, runtimeConfig);
  try {
    const raw = await provider.complete({ task, evidence: task.evidence_manifest, instruction: 'Return only structured application answers grounded exclusively in the supplied evidence.' });
    const response = typeof raw.response === 'string' ? JSON.parse(raw.response) : raw.response;
    return { route, answers: validateGeneratedAnswers(response, task), usage: raw.usage };
  } finally { provider.close?.(); }
}
...
export async function generateSalaryPreferences({ questions, evidence, runtimeConfig }) {
  ...
  const allowedProviders = hostedFallbackProviderIds(runtimeConfig);
  if (!allowedProviders.length) return { route: { result: 'NO_ELIGIBLE_PROVIDER', reason: 'PROVIDER_UNAVAILABLE' }, answers: [], blocker: 'PROVIDER_UNAVAILABLE' };
  const route = routeTask(salaryRoutingEnvelope(task), runtimeConfig, { providerIds: allowedProviders });
  if (route.result !== 'ROUTED') return { route, answers: [], blocker: route.reason };
  const provider = createProvider(route.provider_id, {
    ...runtimeConfig.providers[route.provider_id],
    json_schema_file: new URL('../../schemas/runtime/application-salary-response.v1.schema.json', import.meta.url).pathname,
