...
import { atsFor, enabledAts, isCertifiedAts } from './ats.mjs';
...
async function withCertifiedApplyUrl(candidate, { fetchImpl } = {}) {
  if (!candidate?.canonical_url) return candidate;
  if (isCertifiedAts(atsFor(candidate.canonical_url))) return candidate;
...
function certifyOrReviewQueuedAttempt(target, result) {
  ...
  if (attempt.ats !== effectiveAts && isCertifiedAts(effectiveAts)) {
    return {
...
export async function enqueueEligible(target, { includeCurrent = false, trackerNumbers = null, fetchImpl } = {}) {
  const rows = eligibleRows(target);
  const queued = [];
  const blocked = [];
  const allowed = trackerNumbers ? new Set([...trackerNumbers].map(Number)) : null;
  for (const raw of rows) {
    if (allowed && !allowed.has(raw.row.num)) continue;
    if (!raw.eligible) { blocked.push({ tracker_number: raw.row.num, blocker: raw.blocker }); continue; }
    const candidate = await withCertifiedApplyUrl(raw, { fetchImpl });
    const certified = certifyOrReviewQueuedAttempt(target, attemptFromCandidate(target, candidate));
...
export async function enqueueSelectionOverride(target, trackerNumber, { fetchImpl } = {}) {
  const raw = candidateForTrackerNumber(target, trackerNumber);
...
export async function runApplications(target, config, { submit = false, max = 1, pauseForAuthentication = false, attemptKeys = null, onProgress = null } = {}) {
  const note = (progress = {}) => {
  ...
  for (const stale of listAttempts(target).filter(item => item.state === 'QUEUED'
    ...
    certifyOrReviewQueuedAttempt(target, { attempt: stale, created: false });
  }
  ...
  try {
    ...
    for (const queued of pending) {
      ...
      try {
        const navigation = await page.goto(attempt.canonical_url, { waitUntil: 'domcontentloaded', timeout: 60000 });
        await delay(1800);
        note({
          phase: 'open',
          tracker_number: attempt.tracker_number,
        ...
        if (enabledBoard && isCertifiedAts(inspected.board) && inspected.fieldCount === 0 && !inspected.navigation?.login) {
        ...
        const pageState = { ...inspected.navigation, certified: enabledBoard && isCertifiedAts(inspected.board), exactReviewPage: inspected.navigation.review };
        ...
        for (let step = 1; step <= maxSteps; step++) {
          ...
          if (!enabledBoard || !isCertifiedAts(inspected.board) || !inspected.navigation?.hasNext || inspected.navigation?.review) break;
        ...
        if (custom.length) {
          note({
            phase: 'prose',
            result: `${custom.length} custom fields`,
            company: attempt.company,
            tracker_number: attempt.tracker_number,
          });
        ...
        if (salary.length) {
          note({
            phase: 'salary',
            result: `${salary.length} salary fields`,
            company: attempt.company,
            tracker_number: attempt.tracker_number,
          });
          const evidence = answerEvidence(target, attempt, config);
        ...
        const gate = submissionGate({ page: { ...inspected.navigation, certified: enabledBoard && isCertifiedAts(inspected.board), exactReviewPage }, fields, resume: attachedResume, generated });
