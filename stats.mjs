...
// Status vocab, inlined (mirrors followup-cadence.mjs). Not imported from there:
// that module runs a side-effectful CLI at top level, which an import would fire.
const STATUS_ALIASES = {
  ...
  'rejected-at-eval': 'rejected_at_eval', 'eval-reject': 'rejected_at_eval', 'screened-out': 'rejected_at_eval',
  'purge': 'purged', 'auto-discarded': 'purged', 'age-purged': 'purged', 'liveness-purged': 'purged',
...
// Paths resolve under $CAREER_OPS_DATA_DIR (default ft/); portals.yml is shared root.
const P = resolvePaths(import.meta.url);
const APPS_FILE = P.appsFile;
const SCAN_HISTORY_FILE = join(P.dataDir, 'scan-history.tsv');
const FOLLOWUPS_FILE = join(P.dataDir, 'follow-ups.md');
const SCAN_RUNS_FILE = join(P.dataDir, 'scan-runs.tsv');
const PORTALS_FILE = P.portalsFile;

const CANONICAL_STATUSES = ['Evaluated', 'Applied', 'Responded', 'Interview', 'Offer', 'Rejected', 'Discarded', 'Rejected-at-eval', 'Purged', 'SKIP'];
