...
4. **Subject line: purpose + term + level. No your name.** Example: `Research collaboration inquiry, MISM master's, multi-agent + healthcare ML`. Avoid blank subjects, "Hi Professor", or anything generic.
...
12. **Lead with evidence, calibrate the tone DOWN.** People decide first, rationalize after, but performative excitement *hurts* you when it reads as overclaim. Anmol is a December 2026 grad reaching out about research collaboration or a Fall 2026 RA slot, so the framing should be a working researcher's, not an applicant's. Phrases like "is what I keep coming back to," "I came to CMU specifically to," or "is the closest framing I've found" register as overclaim. Use matter-of-fact framing: "overlaps with a problem I'm stuck on," "Cloudify exposes but does not solve." Let the project specifics carry the signal; do not editorialize about the paper.
...
**Verdict for Anmol's batch:** skipped by default for the initial 12, the labs are pre-researched with specific paper hooks and the timeline is short. **Use as the fallback for any prof who doesn't reply within 7 days:** instead of a follow-up to the PI, ladder via a named grad student in their group.
...
```
Subject: Research collaboration inquiry, [your level], [topic in 4-6 words]
...
"course credit," no "paid." Do NOT say "Available January 2027" or any other
...
- **Story:** 2.5 yrs SDE Byju's → CMU MISM Heinz BIDA → 3 recent projects (Highmark cancer-staging XGBoost on 6M+ claims, finished / Cloudify multi-agent OpenAI+Claude / EEG classification CMU 11-685 multi-head CNN+Transformer + CLIP retrieval).
...
- **Hard rules to honor:** no em-dashes or en-dashes (Rule 1); no F-1/OPT/H-1B/Heinz/OIE/sponsorship explainer (Rule 3); no proactive "Available January 2027" or any other availability statement (memory `feedback_no_availability_phrases.md`); no funding-term mention, neither "paid" nor "volunteer" nor "course credit" (turn-5 user directive 2026-05-06); drafts only, user sends (CLAUDE.md "What I am NOT to do without explicit ask").
- **Tone:** matter-of-fact, not performative. Overclaiming reads as false. Banned phrases: "is what I keep coming back to," "I came to CMU specifically to," "is the closest framing I've found," "fellow [program] student," "to round out the [X] side." Use instead: "overlaps with a problem I'm stuck on," "exposes but does not solve," "I have not figured out / not solved cleanly."
