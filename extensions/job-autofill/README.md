...
Only the resume slot gets a file. Greenhouse renders Resume/CV and Cover Letter as the same upload control, both labelled `Attach` with the same accept list, so filling every file input attached the resume as the cover letter too, and that file goes to the employer. `isResumeInput` reads the id, name and aria-label first, then the nearest text that names the slot, and answers false when nothing does: an upload left for you is cheap, and the wrong document on an application is not. Cover-letter uploads, transcripts, portfolios and writing samples are reported under **Needs you**. Separately, the Career-Ops runner can explicitly reveal Greenhouse's `Enter manually` cover-letter textarea and supply a validated local-prose override when that feature is enabled in ignored local configuration; the extension itself never generates the letter or uploads a substitute document.
